...
if (DEMO_MODE) console.log('⚠️ DEMO MODE – auto‑confirm 5s');
...
app.get('/api/payment-status/:id', async (r,res) => {
  ...
  try {
    ...
    if (status === 'completed' && (resultCode === 0 || resultCode === '0')) {
      ...
      const sales = readDB('sales'); sales.push({ title: scheme?.title, grade: scheme?.grade, phone: tx.phone, amount: scheme?.price, date: new Date().toISOString() }); writeDB('sales', sales); incStat('sales');
...
// ---------- ADMIN (all routes intact, with B2 safeguard) ----------
